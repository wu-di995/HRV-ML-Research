# Generate sparc on linear and angular user command windows
import pandas as pd
import numpy as np
...
for teleopFolder in teleopFolders:
    userCmdPaths.append(glob.glob(teleopFolder+"*user_cmd.csv")[0])
# print(len(userCmdPaths))
...
# Save directory
savedir = "E:\\argall-lab-data\\SPARC_userCmd_byEventNEW\\"
...
# Function to get SPARC for linear, angular user commands
def get_SPARCWindows(userCmd_df, ECGWin_df):
    # Create a dataframe to save SPARC 
    userCmdSPARC_columns = ["Start Win Time", "End Win Time","Lin SPARC", "Ang SPARC"]
    # Dataframe using closest search method for start, and before search method for end
    userCmdSPARC_close_df = pd.DataFrame(columns=userCmdSPARC_columns, index = range(len(ECGWin_df))) 
    # Dataframe using after search method for start, and before search method for end 
    userCmdSPARC_aft_df = pd.DataFrame(columns=userCmdSPARC_columns, index = range(len(ECGWin_df)))
    ...
    for idx,row in ECGWin_df.iterrows():
        ...
        startTime_close, idx_close, startTime_aft, idx_aft = find_closest(userCmd_df,ECGStartTime,"CA",0,test=False,match_annot=False)
        ...
        endTime, idx_end = find_closest(userCmd_df,ECGEndTime,"B",0,False,False)
        if endTime!= None:
            endTime_exist = 1
        ...
        # userCmd_df columns = ["Timestamp", "User Control", "Lin_userCmd", "Ang_userCmd"]
        if startTime_close_exist:
            lin_userCmd_close = userCmd_df.iloc[idx_close:idx_end+1,2].values # Linear user command
            ang_userCmd_close = userCmd_df.iloc[idx_close:idx_end+1,3].values # Angular user command
            # Save start and end times
            userCmdSPARC_close_df.loc[idx,"Start Win Time"] = startTime_close
            userCmdSPARC_close_df.loc[idx,"End Win Time"] = endTime
            # Save SPARC
            ...
            userCmdSPARC_close_df.loc[idx,"Lin SPARC"] = lin_sal
            userCmdSPARC_close_df.loc[idx,"Ang SPARC"] = ang_sal
        if startTime_aft_exist:
            lin_userCmd_aft = userCmd_df.iloc[idx_aft:idx_end+1,2].values # Linear user command
            ang_userCmd_aft = userCmd_df.iloc[idx_aft:idx_end+1,3].values # Angular user command
            # Save start and end times
            userCmdSPARC_aft_df.loc[idx,"Start Win Time"] = startTime_close
            userCmdSPARC_aft_df.loc[idx,"End Win Time"] = endTime
            # Save SPARC
            lin_sal = sparc_window(lin_userCmd_aft,startTime_aft,endTime)
            ang_sal = sparc_window(ang_userCmd_aft,startTime_aft,endTime)
            userCmdSPARC_aft_df.loc[idx,"Lin SPARC"] = lin_sal
            userCmdSPARC_aft_df.loc[idx,"Ang SPARC"] = ang_sal
    return userCmdSPARC_close_df, userCmdSPARC_aft_df

# For each userCmd file, read the ECG30s windows and ECG60s windows dataframes 
...
if len(ECG30Paths) == len(ECG60Paths):
    for userCmdPath in userCmdPaths:
        event = readEvent(userCmdPath)
        print(event)
        ECG30Path = [path for path in ECG30Paths if event in path][0]
        ECG60Path = [path for path in ECG60Paths if event in path][0]
        # Read datframes
        userCmd_df = pd.read_csv(userCmdPath)
        ECG30_df = pd.read_csv(ECG30Path)
        ECG60_df = pd.read_csv(ECG60Path)
        # Generate user control dataframes 
        print("30")
        userCmd30_close_df, userCmd30_aft_df = get_SPARCWindows(userCmd_df,ECG30_df)
        print("60")
        userCmd60_close_df, userCmd60_aft_df = get_SPARCWindows(userCmd_df,ECG60_df)
        # Save dataframes
        userCmd30_close_df.to_csv(savedir+event+"_close_30.csv")
        userCmd30_aft_df.to_csv(savedir+event+"_aft_30.csv")
        userCmd60_close_df.to_csv(savedir+event+"_close_60.csv")
        userCmd60_aft_df.to_csv(savedir+event+"_aft_60.csv")
